...
__shared__ float Nds[TILE_WIDTH][TILE_WIDTH];
 ...
 int ty=threadIdx.y;
	...
	for (int ph = 0; ph < (x2+TILE_WIDTH-1) / TILE_WIDTH ; ++ph) {
		...
		if (ph * TILE_WIDTH + threadIdx.x < x2 && Row < x1)
			Mds[ty][tx] = d_M[Row * x2 + ph * TILE_WIDTH + tx];
		else
			Mds[ty][tx] = 0.0;

		 
		if (ph * TILE_WIDTH + threadIdx.y < x2 && Col < x3)
			Nds[ty][tx] = d_N[(ph * TILE_WIDTH + ty) * x3 + Col];
		else
			Nds[ty][tx] = 0.0;
...
void matrix_ParMul(float* h_A, float* h_B, float* h_C2, int x1, int x2, int x3)
{
  ...
  dim3 dimBlock(TILE_WIDTH , TILE_WIDTH);
	dim3 dimGrid(ceil(x3 / float( dimBlock.x)), (ceil(x1 / float(dimBlock.y))));
