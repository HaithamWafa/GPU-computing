...
__global__ void SumReduceKernel_DIV(float* X) {
	...
	for (stride = blockDim.x/2; stride >= 1; stride = stride>> 1)
	{
		__syncthreads();
		if (t< stride)
...
float D_SumReduce(float* h_X)
{
	...
	int N = VEC_SIZE;
	while (N > 1) {
		SumReduceKernel_DIV << < dimBlock, BLOCK_WIDTH >> > (d_X);
		N = ceil(N / (float(VEC_SIZE)/BLOCK_WIDTH));
	}
...
int main(void)
{
	...
	printf("The below computations are for reduced divergence kernel:\n");
